...
/**
 * Profile fields a signed-in non-staff user may update on existing rows.
 */
const PUBLIC_SOFT_KEYS = [
  "name",
  "username",
  "passwordHash",
  "birthDate",
...
/**
 * Per-account learning progress. Only accepted for the signed-in user's
 * own row on the public (non-staff) accounts path. Without these keys,
 * studied / favorites / SRS were silently stripped on every
 * saveAccountsOnly from a regular user — so progress never reached the DB
 * or other devices.
 */
const PUBLIC_PROGRESS_KEYS = [
  "studied",
  "studiedAt",
  "favorites",
  "srsStats",
 ...
 * @param {string} [actorCode] - signed-in user code; progress keys are only
 *   accepted when the row being updated is this actor's own account.
 * @returns {object[]} sanitized list to merge
 */
export function sanitizePublicAccountsMerge(nextIncoming, currentAccounts, actorCode = "") {
  const existing = new Map(
    (currentAccounts || [])
      .filter((a) => a && a.code)
      .map((a) => [String(a.code), a])
  );
  const selfCode = actorCode ? String(actorCode) : "";
  const out = [];
  for (const a of Array.isArray(nextIncoming) ? nextIncoming : []) {
    ...
    if (prev) {
      // SECURITY: a non-staff actor may only modify their OWN existing row.
      // Any other existing account included in the incoming array is ignored
      // entirely on this public path — otherwise an authenticated (or even
      // unauthenticated) caller could overwrite another user's passwordHash /
      // email / sessionId by including that user's code (account takeover).
      if (!selfCode || key !== selfCode) {
        continue;
      }
      // Start from DB row so bacTrack / social link / avatar never get dropped
      // when the client sends a partial or light account object.
      const soft = { ...prev };
      const allowedKeys = PUBLIC_SOFT_KEYS.concat(PUBLIC_PROGRESS_KEYS);
