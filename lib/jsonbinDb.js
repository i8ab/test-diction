...
export async function loadRecord() {
  ...
  return { entries, accounts, logs, siteBanner, examConfig, academicUnits, unitStructure, version };
}
...
/**
 * Bulk upsert for a SMALL, EXPLICIT list of entries (e.g. one page of an AI/PDF
 * import) — unlike saveEntriesSync this never compares against the full DB id
 * set and never deletes orphans. Only the entries passed in are touched:
 *   - ids already in the DB → PATCH (update in place).
 *   - new ids → INSERT, batched 50 rows per POST (same as saveEntriesSync).
 * This is what lets importing 400-500 words stay fast and safe: each request
 * only pays for the words actually being added, never for the rest of an
 * existing, possibly-large dictionary.
 */
export async function bulkUpsertEntriesPartial(entries) {
  const list = Array.isArray(entries) ? entries : [];
  if (!list.length) return { patched: 0, inserted: 0 };
  ...
  if (!byId.size) return { patched: 0, inserted: 0 };

  // Only need to know which of THESE ids already exist — no full-table scan.
  const idList = [...byId.keys()];
  const existing = new Set();
  const CHECK_CHUNK = 100;
  for (let i = 0; i < idList.length; i += CHECK_CHUNK) {
    const chunk = idList.slice(i, i + CHECK_CHUNK);
    const orFilter = chunk
      .map((id) => `data->>id.eq.${encodeURIComponent(id)}`)
      .join(",");
    try {
      const rows = await sbFetch(
        "GET",
        `entries?select=data->>id&or=(${orFilter})`
      );
      for (const r of rows || []) {
        if (r && r.id != null) existing.add(String(r.id));
      }
    } catch (_) {
      // If the existence check itself fails, fall through and treat every
      // id in this chunk as new — patchEntryByWordId below still safely
      // no-ops → insert for anything that turns out to already exist is
      // avoided by patchEntryByWordId's own upsert-if-missing behavior.
    }
  }
  ...
  const toInsert = [];
  for (const [id, e] of byId) {
    if (existing.has(id)) toPatch.push(e);
    else toInsert.push(e);
  }
...
export async function saveFullRecord(record, nextVersion) {
  ...
  await sbFetch(
    ...
    [
      ...
      { key: "unit_structure", value: record.unitStructure || null },
    ],
    { Prefer: "resolution=merge-duplicates,return=minimal" }
  );
  await invalidateHotCaches();
...
export async function buildConflictPayload(scope = "full", extra = {}) {
  ...
  // Entry scopes: return full entries so the client can re-apply local ops
  // on top of server state without wiping the dictionary (critical for 409).
  if (s === "entrypatch" || s === "entriesbulkpatch" || s === "entrydelete" || s === "entries") {
    try {
      const rows = await sbFetch("GET", "entries?select=data");
      const entries = (rows || []).map((r) => r.data).filter(Boolean);
      return { ...base, entries };
    } catch (_) {
      return base;
    }
  ...
  // Settings / logs patches: client already has local data; just give version.
  if (s === "settingspatch" || s === "logsreplace") {
    return base;
  }
