...
// Validates/sanitizes the Unit → Section → Lesson template (same shape for
// every academic unit). Mirrors pickAcademicUnits: unknown/malformed shapes
// collapse to null so the caller falls back to the client-side default.
export function pickUnitStructure(raw) {
  ...
  for (const s of raw.sections) {
    ...
    if (!id || seenSection.has(id)) continue;
    const lessonsRaw = Array.isArray(s.lessons) ? s.lessons : [];
    const seenLesson = new Set();
    const lessons = [];
    for (const l of lessonsRaw) {
      if (!l || typeof l !== "object") continue;
      const lid = typeof l.id === "string" && l.id.trim() ? l.id.trim() : null;
      if (!lid || seenLesson.has(lid)) continue;
      seenLesson.add(lid);
      const number =
        typeof l.number === "number" && Number.isFinite(l.number)
          ? l.number
          : lessons.length + 1;
      const label =
        typeof l.label === "string" && l.label.trim() ? l.label.trim() : `Lesson ${number}`;
      const customName = typeof l.customName === "string" ? l.customName.trim().slice(0, 120) : "";
      lessons.push({ id: lid, number, label, customName });
    }
    if (!lessons.length) continue;
    seenSection.add(id);
    const label = typeof s.label === "string" && s.label.trim() ? s.label.trim() : `Section ${id}`;
    const customName = typeof s.customName === "string" ? s.customName.trim().slice(0, 120) : "";
    sections.push({
      id,
      label,
      customName,
