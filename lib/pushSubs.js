...
/** Hard cap — prevents zombie endpoints from piling up after repeated
 *  re-subscribes. Real users rarely have more than a few phones. */
const MAX_DEVICES_PER_ACCOUNT = 2;

/**
 * Add or replace a subscription for this device endpoint.
 * Keeps at most MAX_DEVICES_PER_ACCOUNT (newest wins when over the cap).
...
export async function upsertSub(code, subscription) {
  ...
  // Drop oldest first if over the cap (array order = oldest → newest)
  const trimmed =
    next.length > MAX_DEVICES_PER_ACCOUNT
      ? next.slice(next.length - MAX_DEVICES_PER_ACCOUNT)
      : next;
  return saveSubs(code, trimmed);
}
