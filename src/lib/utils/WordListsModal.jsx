...
export default function WordListsModal({
  ...
  function handleExport(list) {
    const words = (list.entryIds || [])
      .map((id) => entries.find((e) => e.id === id))
      .filter(Boolean);
    exportWordListPdf(words, { title: list.name, section });
  }
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <HowItWorksButton isAr={isAr} guideId="lists" />
          ...
          {lists.map((l) => (
            <div key={l.id} style={{ border: "1px solid rgba(var(--border-rgb),0.14)", borderRadius: 12, padding: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                ...
                <button type="button" onClick={() => handleExport(l)} title={tr(isAr, "Export PDF", "تصدير PDF")} style={{ border: "none", background: "var(--input-bg)", borderRadius: 10, width: 36, height: 36, cursor: "pointer", color: BRASS }}>
